...
/*!
    ...
    thread wait for a Semaphore_t Post.  That operation would then invoke the
    UnBlock() method, removing the blocking thread from the Semaphore_t's list,
...
//---------------------------------------------------------------------------
/*!
    \fn void BlockingObject_Block(Thread_t *pstThread_);

    \param pstList_ Pointer to the threadlist associated with the blocking
                    object, that will receive the thread as a result of
                    the operation.

    \param pstThread_ Pointer to the thread object that will be blocked.
...
//---------------------------------------------------------------------------
/*!
    \fn void BlockingObject_UnBlock(Thread_t *pstThread_)

    \param pstThread_ Pointer to the thread to unblock.
