...
/*
    ...
    This handler has the job of taking the first Thread_t object's stack, and
    restoring the default state data in a way that ensures that the Thread_t
    starts executing when returning from the call.

    We also keep in mind that there's an 8-byte offset from the beginning of
    the Thread_t object to the location of the Thread_t stack pointer.  This
	offset is a result of the Thread_t object inheriting from the linked-list
    ...
    get_thread_stack:
        ; Get the stack pointer for the current Thread_t
        ldr r0, g_pstCurrent
        ldr r1, [r0]
        add r1, #8
    ...
    ** Note - Since we don't care about these registers on init, we could take a shortcut if we wanted to **
    ...
    set_lr:
        ; Set up the link register such that on return, the code operates in Thread_t mode using the PSP
    ...
    exit_exception:
        ...
        ; for us.  If all goes well, our Thread_t will start execution at the entrypoint, with the user-specified
        ; argument.
        bx r0
...
{
	ASM(
	// Get the pointer to the first Thread_t's stack
	...
	" sub r2, #32 \n "
	" ldmia r2!, {r4-r7} \n "
	// Also modify the control register to force use of Thread_t mode as well
	// For CM3 forward-compatibility, also set user mode.
	" mrs r0, control \n"
	" mov r1, #0x03 \n"
	" orr r0, r1 \n"
	" msr control, r0 \n"	
	// Return into Thread_t mode, using PSP as the Thread_t's stack pointer
...
/*
	...
	1) Save Context
	2) Swap "current" and "next" Thread_t pointers
	3) Restore Context
...
1) Saving the context.
	...
	; Get address of current Thread_t stack
	...
	Thread_t's top-of-stack value is stored, all registers are stacked.  We're good to go!

2)  Swap threads

	This is the easy part - we just call a function to swap in the Thread_t "current" Thread_t
	from the "next" Thread_t.
...
{	
	...
	// Get the pointer to the next Thread_t's stack
	" add r0, #8 \n "
	" ldr r2, [r0] \n "
